...
/** Whether a pre-step rejection only pauses work for context compaction. */
function isTransientCompactionRejection(decision) {
	...
	ctx.effect(function* () {
		...
		ctx.on("agent/pre-step", async ({ agent, messages, signal }, next) => {
			...
			if (decision.kind === "reject") {
				state.attempt = void 0;
				if (isTransientCompactionRejection(decision)) {
					// Context compaction intentionally rejects the current admission;
					// the goal remains active and must be retried after the guard settles.
					requestDrive(state);
					return decision;
				}
				const goal = currentGoal(state);
