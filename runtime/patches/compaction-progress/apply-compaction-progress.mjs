...
export function findTarget(explicit) {
  ...
  for (const anchor of anchors) {
    try { return realpathSync(join(dirname(createRequire(anchor).resolve(`${manifest.package}/package.json`)), manifest.relativeTarget)); } catch {}
  }
  throw new Error('Could not resolve chat client; pass --target PACKAGE_DIRECTORY or lib/client.js.');
}
...
export function applyPatch({ target, check = false } = {}) {
  ...
  const backup = `${destination}.before-compaction-progress-${manifest.baseSha256.slice(0, 12)}`;
  ...
  const temporary = `${destination}.compaction-progress-${randomUUID()}.tmp`;
  try {
  ...
  try {
    ...
    for (let index = 0; index < args.length; index += 1) {
      if (args[index] === '--check') check = true;
        ...
        process.stdout.write('Usage: node apply-compaction-progress.mjs [--target PACKAGE_DIR_OR_CLIENT_JS] [--check]\n--check is read-only: exit 0 patched, exit 2 unpatched, exit 1 unsupported/error.\n');
        process.exit(0);
