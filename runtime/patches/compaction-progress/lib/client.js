window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function CompactionProgress({ node }) {
			const failed = node.progressError !== undefined;
			const summarized = node.progressPhase === "saving";
			const stages = [
				["Leitura e resumo do contexto", summarized ? "complete" : failed ? "error" : "active"],
				["Compactação e gravação", summarized ? failed ? "error" : "active" : "pending"]
			];
			return (0, react_jsx_runtime.jsxs)("section", {
				"aria-label": "Progresso da compactação",
				style: { padding: "12px 0", display: "grid", gap: 10, color: "var(--dsw-alias-label-secondary)" },
				children: [
					...stages.map(([label, status]) => (0, react_jsx_runtime.jsxs)("div", {
						children: [
							(0, react_jsx_runtime.jsx)("div", { style: { fontSize: 13, marginBottom: 5 }, children: label + " · " + ({ complete: "concluída", active: "em andamento", pending: "aguardando", error: "interrompida" })[status] }),
							(0, react_jsx_runtime.jsx)("progress", { "aria-label": label, max: 1, ...(status === "active" ? {} : { value: status === "complete" ? 1 : 0 }), style: { display: "block", width: "100%", height: 8, accentColor: "var(--dsw-alias-button-info-fill, #6288ee)" } })
						]
					}, label)),
					(0, react_jsx_runtime.jsx)("div", { role: "status", style: { fontSize: 12 }, children: failed ? "Compactação interrompida: " + node.progressError : "O percentual não é fornecido pelo servidor. A etapa muda quando o resumo fica pronto." })
				]
			});
		}
		/**
		...
		*/
		const CompactionItem = (0, react.memo)(function CompactionItem({ node, title, fallbackSummary, t }) {
			...
			if (node.progressPhase) return (0, react_jsx_runtime.jsx)(CompactionProgress, { node });
		...
		*/
		...
		function compactionView(state) {
			if (state.ended && state.ended.event.data.error === undefined && state.checkpoint) return compactSummary(state.summary, state.checkpoint);
			if (!state.started && !state.ended) return state.checkpoint ? compactSummary(state.summary, state.checkpoint) : null;
			...
			return {
				...
				progressPhase: state.summary || state.checkpoint ? "saving" : "reading",
				...(state.ended ? { progressError: state.ended.event.data.error ?? "Termino sem checkpoint confirmado" } : {})
			};
		}
		...
		const commandDefinition = {
			...
			buildViewNode: (context) => {
				...
				const compaction = compactionView(state);
				const data = {
		...
		*/
		...
		function fallbackState$3(context) {
			...
			const checkpoint = context.matches.find((match) => compactSource(match.event) !== void 0);
			return {
				started: context.matches.find((match) => match.event.type === "compaction/start"),
				ended: context.matches.find((match) => match.event.type === "compaction/end"),
				...summary === void 0 ? {} : { summary },
		...
		const compactionDefinition = {
			...
			buildViewNode: (context) => {
				...
				const marker = compactionView(state);
