...
/** Preserve the configured local inference idle window at the HTTP layer. */
function localProfileFetch(profile) {
	let origin;
	try {
		const url = new URL(profile.baseURL);
		if (!['localhost', '127.0.0.1', '[::1]'].includes(url.hostname) || !['http:', 'https:'].includes(url.protocol)) return undefined;
		origin = url.origin;
	} catch { return undefined; }
	const idle = profile.streamIdleTimeoutMs;
	if (!Number.isSafeInteger(idle) || idle <= 0) return undefined;
	const headers = Number.isSafeInteger(profile.timeoutMs) && profile.timeoutMs > 0 ? profile.timeoutMs : idle;
	return (input, init = {}) => {
		const upstream = init.dispatcher ?? getGlobalDispatcher();
		const dispatcher = {
			dispatch(options, handler) {
				// Redirects to another origin retain their existing transport policy.
				return upstream.dispatch(String(options.origin) === origin ? { ...options, headersTimeout: headers, bodyTimeout: idle } : options, handler);
			}
		};
...
function profileOptions(profile, reasoning, apiKey) {
	...
	const localFetch = localProfileFetch(profile);
	return {
		...localFetch === undefined ? {} : { fetch: localFetch },
