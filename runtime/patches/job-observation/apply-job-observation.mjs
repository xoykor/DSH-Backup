...
export function findTarget(explicit) {
  ...
  throw new Error('Could not resolve @deepseek-ai/dsh-tool-jobs; pass --target /path/to/package or /path/to/lib/index.js.');
}
...
export function applyPatch({ target, check = false } = {}) {
  ...
  const backup = `${destination}.before-job-observation-${manifest.baseSha256.slice(0, 12)}`;
  ...
  const temporary = `${destination}.job-observation-${randomUUID()}.tmp`;
  try {
  ...
  try {
    ...
    for (let index = 0; index < args.length; index += 1) {
      if (args[index] === '--check') check = true;
        ...
        process.stdout.write('Usage: node apply-job-observation.mjs [--target PACKAGE_DIR_OR_INDEX_JS] [--check]\n--check is read-only: exit 0 patched, exit 2 unpatched, exit 1 unsupported/error.\n');
        process.exit(0);
