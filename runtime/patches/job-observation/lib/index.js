...
// Host-owned capability on the registered ToolDefinition, never model arguments.
const JOB_OBSERVATION = Symbol.for("dsh.executor.jobObservation.v1");
function withJobObservation(tool, observe) {
	Object.defineProperty(tool, JOB_OBSERVATION, {
		value: Object.freeze({ readOnly: true, observe }),
		enumerable: false,
		writable: false,
		configurable: false
	});
...
function jobOutputSuffix(value) {
	return `${value.waitExpired === true ? "\n[wait expired; job remains active]" : ""}\n${statusLine(value.job)}`;
}
...
function apply(ctx, config) {
	...
	const finalizeTaskContent = (exec, result) => {
		...
		if (exec.name === "job_output" && !result.isError) {
			...
			const suffix = jobOutputSuffix(value);
	...
	ctx.tools.register(withJobObservation(defineTool({
		name: "job_output",
		...
		parameters: {
			...
			wait: {
				type: "boolean",
				description: "Block until the job reaches a terminal status or the timeout expires. An expired wait returns waitExpired: true while the job remains active; it is not an execution timeout."
			},
			timeout_ms: {
				type: "number",
		...
		output: {
			...
			render: (_args, value) => {
				...
				return [{
					type: "text",
					text: `${body.endsWith("\n") ? body.slice(0, -1) : body}${jobOutputSuffix(value)}`
				}];
			}
		},
		async execute(args, exec) {
			...
			return {
				...
				...args.wait === true ? { waitExpired: read.snapshot.status === "running" || read.snapshot.status === "stopping" } : {}
			};
		},
		...
		if (typeof args.job_id !== "string" || args.job_id.length === 0) return void 0;
		if (args.wait !== void 0 && typeof args.wait !== "boolean") return void 0;
		if (args.timeout_ms !== void 0 && (!Number.isFinite(args.timeout_ms) || args.timeout_ms <= 0)) return void 0;
		if (Object.keys(args).some((key) => key !== "job_id" && key !== "wait" && key !== "timeout_ms")) return void 0;
		const jobId = validateJobId(args.job_id);
		const snapshot = ctx.jobs.get(jobId, exec.agent);
		return {
			kind: "output",
			jobId,
			status: snapshot.status,
			waitMs: args.wait === true ? Math.min(args.timeout_ms ?? waitDefault, waitCap) : 0
		};
	}));
	ctx.tools.register(withJobObservation(defineTool({
		name: "job_list",
