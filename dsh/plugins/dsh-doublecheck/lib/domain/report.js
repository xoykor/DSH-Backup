        ...
        switch (event.type) {
            case 'tool/call': {
                ...
                if (event.data.name === SPEC_TOOL_NAME) {
                    ...
                    if (spec !== null)
                        pendingSpecs.set(event.data.callId, spec);
                }
 ...
 */
export function deriveReportVerdict(facts, verification) {
    if (facts.spec === null)
        return 'grill';
    if (facts.edits === 0)
        return 'draft';
    ...
    if (verification !== null) {
        if (verification.checks.some(check => check.verdict === 'fail'))
            return 'challenged';
 ...
 */
export function buildVerifyScript(mode) {
    if (mode === 'single') {
        return [
