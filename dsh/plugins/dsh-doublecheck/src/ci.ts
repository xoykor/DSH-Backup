...
import { renderGateReportJson, renderGateReportSarif, type GateState } from './domain/gate.ts'

/** Parse CLI arguments into `{ format, input }`. */
function parseArgs(argv: string[]): { format: 'json' | 'sarif'; input: string | null } {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--format' || arg === '-f') {
      ...
      if (next !== 'json' && next !== 'sarif') throw new Error(`--format must be json or sarif, got ${JSON.stringify(next)}`)
      format = next
      index += 1
    } else if (arg === '--input' || arg === '-i') {
      input = argv[index + 1] ?? null
      if (input === null) throw new Error('--input requires a file path')
      index += 1
...
function readState(input: string | null): GateState {
