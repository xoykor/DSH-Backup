...
flowchart LR
  ...
  pkg_web_search_searxng["dsh-web-search-searxng"]
...
| `ctx.web` | `seam` | [`web`](../packages/web/web) | `dsh-web-search-searxng`, [`web-fetch-http`](../packages/web/web-fetch-http) | [`tool-web`](../packages/web/tool-web) | - | 此部署只注册 SearXNG 搜索和 HTTP 抓取；tool-web 负责稳定的面向模型名称。 |
