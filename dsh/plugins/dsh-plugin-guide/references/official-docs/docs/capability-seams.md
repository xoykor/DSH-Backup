...
| `ctx.web` | `seam` | [`web`](../packages/web/web) | `dsh-web-search-searxng`, [`web-fetch-http`](../packages/web/web-fetch-http) | [`tool-web`](../packages/web/tool-web) | - | This deployment registers only SearXNG for search and the HTTP backend for fetch; tool-web owns the stable model-facing names. |
