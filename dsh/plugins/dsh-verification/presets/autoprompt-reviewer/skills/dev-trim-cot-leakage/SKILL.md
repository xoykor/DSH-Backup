---
name: dev-trim-cot-leakage
description: Use when auditing or fixing prose that reads like a leaked reasoning transcript — dead design-session citations such as (decision N), audit item codes, or §N of uncommitted drafts; change narration such as "used to", "no longer", "this cut"; stack or review vantage ("a later PR in this stack", "rejected in review"); reviewer-addressed justifications; control-flow narration; or hedged planning residue in comments, JSDoc, docs, or design notes.
---

# Trimming Chain-of-Thought Leakage

Chain-of-thought leakage is prose whose vantage is the authoring session rather than the repository: it cites artifacts only that session could see, narrates the change instead of the state, or argues with a reviewer who has left. The fix is never deletion alone when a passage carries factual clauses — restate each so it stands at HEAD, then delete the transcript around it; a passage carrying none (an audit code, control-flow narration) is deleted outright. **REQUIRED BACKGROUND:** [dev-prose-standard](../dev-prose-standard/SKILL.md) owns the complete-proposition rule this skill applies. It is guidance, not a script.
...
- **Merged-PR and issue citations inside design notes and postmortems** — sanctioned evidence where the project's own documentation standard routes change stories.
...
1. Scope and exclusions per [dev-prose-standard](../dev-prose-standard/SKILL.md): require an explicit scope; never touch vendored/generated directories, archived snapshots, or recorded fixtures — recorded model output and sealed history keep their original voice.
2. Audit read-only first: search for the taxonomy signals (ordinal citations, "used to"/"no longer", "this PR/cut", "rejected in review", control-flow narration, stray working-language fragments) with ripgrep, then judge every hit semantically. Patterns are probes, not the definition — also read the densest prose in scope (module JSDoc, READMEs, design notes) without a pattern in hand.
...
5. Verify: re-run the searches expecting only sanctioned keeps; confirm every remaining citation resolves at HEAD; run the project's relevant gates (`git diff --check`, lint, doc checks where defined).
