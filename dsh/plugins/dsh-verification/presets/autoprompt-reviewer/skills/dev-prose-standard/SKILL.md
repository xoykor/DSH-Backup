...
Always exclude vendored or generated directories (`vendor/`, `node_modules/`, build output, lockfiles) from discovery, review, and edits, even when the requested scope is the whole repository. Do not follow a symlink into them. Put exclusions after inclusion globs so a later include cannot re-admit them: for example, end ripgrep commands with `--glob '!vendor/**'`, and give Git commands an explicit `:(exclude)vendor/**` pathspec. If the requested scope contains only excluded paths, report that no eligible files remain.

Treat generated catalogs, snapshots, and fixtures as derivative. Edit the owning source or generator first, then regenerate the artifact. When a generator extracts a summary from owner prose, make the extracted sentence complete for that surface.
...
- **Design notes / ADRs:** retain unique rationale, mechanisms, alternatives, consequences, verification evidence, and named coverage gaps. Shipped notes state shipped reality in the present tense; remove planning checklists, not evidence of what pins the decision.
...
1. Confirm the scope, mode, current branch or PR base, and applicable project standards (AGENTS.md, CONTRIBUTING.md, style guide) when present. Do not inspect unrelated branches.
2. Read the owning code or document before judging a passage. For calibration or unfamiliar cases, read the distilled examples if the repository keeps any.
...
6. Run the narrow relevant checks for the project (lint, `git diff --check`, behavior tests for visible strings, documentation gates when the repository defines them). Verify the final diff contains no excluded path and report any accidental match rather than claiming a clean exclusion history.
...
After the user decides, distill the principle and versions into the repository's calibration examples (without PR history or reviewer narration) and apply the learned rule to every analogous passage in scope.
