...
export function apply(ctx) {
  // The resident shell differs by platform: Windows uses DSH's official pwsh
  // stack (no bash runner on win32), POSIX keeps the Minimal bash pair.
  ...
  ctx.tools.register({
    ...
    description: [
      ...
      '',
      `This session starts with a minimal resident set: ${shellName}, str_replace_editor, skill_search, skill_load. Everything else is unlocked on demand through this tool.`,
      '',
      'If the current task needs any of the following, call dev_tool_search FIRST — do not try to work around them with the shell:',
