...
const ALLOWED_KEYS = new Set(['bootstrapTools', 'promoteOn', 'bootstrapMaxTokens', 'compactionTools', 'includeSubagents', 'residentTools'])
...
export function apply(ctx, config) {
  ...
  // Autoprompt: protocol tools kept resident across the promoted phase AND
  // the post-compaction controlled phase (seven-phase dispatch surface).
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    try {
      ...
      if (status.promoted) {
        ...
        const keep = new Set([...bootstrapTools, ...RESIDENT_DISCOVERY_TOOLS, ...unlockedFor(context.agent?.session), ...residentTools])
      ...
      if (boundary >= 0) for (const toolName of [...compactionTools, ...residentTools]) keep.add(toolName)
