...
// cordis.yml (service row + plugin row + config), then asserts the plugin's
// contribution and one config-dependent fact. Config is applied by the Loader,
// so the expected outcome proves the config in the file was honored.
//
...
try {
  ...
  // The plugin mounts its BudgetService (the `budget` service) and applies the
  // config; the currency code pins that the Loader applied the file's config.
  const budget = ctx.get('budget')
  if (budget === undefined) throw new Error('Loader composition: budget service is missing')
  const currency = budget.status().currency
  process.stdout.write(`DSH_LOADER_RESULT ${JSON.stringify({ currency: currency.code })}`)
} catch (error) {
