...
3. Execute `scripts/table_transform --input IN --output OUT --operations OPS.json`. O helper Rust preserva a mesma interface. Use `--input-format` e `--output-format` quando a extensão não for suficiente.
...
Dependência: binário Rust distribuído com a skill. O arquivo original não é modificado; a pasta de destino pode ser criada pelo helper.

## Reconciliação de duas tabelas

Use `scripts/table_reconcile` para comparar fontes por uma chave indicada pelo usuário. O transformador acima mantém sua interface; a reconciliação produz relatório JSON separado, sem deduplicar os dados.
...
```bash
scripts/table_reconcile --left antes.csv --right depois.csv --spec comparacao.json --output divergencias.json
```
