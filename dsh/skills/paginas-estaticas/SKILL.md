...
Rode `scripts/check_site --root DIRETORIO --entry index.html`. O helper Rust verifica referências locais em `href`, `src` e `url(...)`, detecta caminhos que escapam da raiz e avisa quando falta uma meta viewport. Um relatório verde comprova integridade estrutural, não aparência.
